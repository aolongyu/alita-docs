...
previousUrl: '/react/quickstart'
nextText: 'Taking Photos'
nextUrl: '/react/your-first-app/2-taking-photos'
---
...
* One React-based codebase that runs on the web, iOS, and Android using Ionic Framework [UI components](https://ionicframework.com/components).
...
* Photo Gallery functionality powered by the Capacitor [Camera](https://capacitor.ionicframework.com/apis/camera), [Filesystem](https://capacitor.ionicframework.com/apis/filesystem), and [Storage](https://capacitor.ionicframework.com/apis/storage) APIs.
...
>
> Consider setting up npm to operate globally without elevated permissions. See [Resolving Permission Errors](/faq/tips#resolving-permission-errors) for more information.
...
```shell
$ alitastart photo-gallery tabs --type=react --capacitor
```
...
Some Capacitor plugins, including the Camera API, provide the web-based functionality and UI via the Ionic [PWA Elements library](https://github.com/alitajs/alita-pwa-elements).
...
![Before and after going through this tutorial](/assets/img/guides/first-app-cap-ng/email-photogallery.gif)
...
We put the visual aspects of our app into `<IonContent>`. In this case, it’s where we’ll add a button that opens the device’s camera as well as displays the image captured by the camera. Start by adding a [floating action button](https://ionicframework.com/api/fab) (FAB). First, update the imports at the top of the page to include the Camera icon as well as the some Ionic components we'll use shortly:
